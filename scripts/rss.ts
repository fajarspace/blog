...
const main = () => {
  const feed = new RSS({
    title: 'Fajar A.',
    site_url: 'https://fajarr.space',
    feed_url: 'https://fajarr.space/feed.xml',
    // image_url: 'https://fajarr.space/og.png',
    language: 'en',
    description: "Fajar A.'s blog",
  })

  posts.forEach((post) => {
    const url = `https://fajarr.space/blog/${post.slug}`
